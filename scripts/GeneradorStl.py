...
def postprocess_with_meshlab(vertices: np.ndarray, faces: np.ndarray, out_path: Path, *, targetperc: float = 0.5, taubin_iters: int = 25, taubin_lambda: float = 0.5, taubin_mu: float = -0.53, close_holes_maxsize: int = 200) -> dict:
    if not _HAS_PYMESHLAB: raise RuntimeError("PyMeshLab no disponible.")
    ms = ml.MeshSet()
    ...
    ok_smooth = False
    try:
        ms.apply_filter('apply_coord_taubin_smoothing', stepsmoothnum=taubin_iters, lambda_=taubin_lambda, mu=taubin_mu)
        ok_smooth = True
    except Exception:
        try:
            ms.apply_filter('apply_coord_laplacian_smoothing_surface_preserving', stepsmoothnum=max(10, taubin_iters), cotangentweight=True)
            ok_smooth = True
        except Exception:
            pass
    if not ok_smooth: raise RuntimeError("No se pudo suavizar (Taubin/alterno).")
    ms.apply_filter('meshing_decimation_quadric_edge_collapse',
                    targetperc=targetperc, preservenormal=True, planarquadric=True,
                    optimalplacement=True, qualitythr=0.3, boundaryweight=1.0, preservetopology=True)
    try:
        ms.apply_filter('apply_coord_taubin_smoothing', stepsmoothnum=max(5, taubin_iters // 5), lambda_=0.5, mu=-0.53)
    except Exception:
        pass
    ...
    for idx, f in enumerate(files, start=1):
        ...
        try:
            ...
            if _HAS_PYMESHLAB:
                ...
                try:
                    metrics = postprocess_with_meshlab(verts, faces, out_stl_hq,
                                                       targetperc=0.65, taubin_iters=60,
