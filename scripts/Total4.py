...
# *Eliminado dcm2niix*. Solo flujo con dicom2nifti (relajado) + fallback decompresión.
...
def robust_dicom_to_nifti(input_dir: Path, work_tmp: Path, on_log=print) -> Path:
    with step(on_log, "NIFTI", "Conversión robusta DICOM→NIfTI"):
        ...
        # ¡Sin dcm2niix!: vamos directo a dicom2nifti relajado
...
def run_totalsegmentator_gpu_then_cpu(input_nii: Path, output_dir: Path,
    ...
    env.setdefault("TQDM_DISABLE", "1")  # quita barras tqdm en consola

    cmd_gpu = build_cmd_gpu(input_nii, output_dir, extra_flags, fast_gpu)
    with step(on_log, "TS|GPU", "Ejecución"):
    ...
    flag(on_log, "TS|GPU", f"rc={rc}", "Fallback a CPU sin --fast")
    cmd_cpu = build_cmd_cpu(input_nii, output_dir, extra_flags)
    with step(on_log, "TS|CPU", "Ejecución"):
...
def preflight_or_fail(on_log):
    with step(on_log, "START", "Preflight"):
        ...
        # Eliminado: chequeo/aviso de dcm2niix.
...
def run_pipeline(
    ...
    flag(log, "START", "TotalSegmentator (GPU→CPU, Ortopedia-only)")
    ...
    flag(log, "END", "Duración total", format_duration(elapsed))
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("SOM3D - TotalSegmentator (GPU→CPU, Ortopedia-only)")
